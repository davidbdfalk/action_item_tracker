class CommentsController < ApplicationController
  ...
  def index
    ...
    @comments = @q.result(:distinct => true).includes(:commenter, :action_item).page(params[:page]).per(10)
  ...
  def create_row
    ...
    @comment.action_item_id = params.fetch("action_item_id")
  ...
  def create_row_from_action_item
    @comment = Comment.new
    ...
    if @comment.valid?
      ...
      redirect_to("/action_items/#{@comment.action_item_id}", notice: "Comment created successfully.")
    else
      render("comment_templates/new_form_with_errors.html.erb")
    end
  end
  ...
  def destroy_row_from_action_item
    @comment = Comment.find(params.fetch("id_to_remove"))

    @comment.destroy

    redirect_to("/action_items/#{@comment.action_item_id}", notice: "Comment deleted successfully.")
  end
